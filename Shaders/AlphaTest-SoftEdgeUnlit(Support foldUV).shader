...
Shader "Hortor/Transparent/Cutout/Soft Edge Unlit(Support foldUV)" {
Properties {
...
SubShader {
    ...
    Pass {
        CGPROGRAM
            ...
            {
                float2 coord = frac(i.texcoord) * i.texcoord3 + i.texcoord2;
                half4 col = _Color * tex2D(_MainTex, coord);
                fixed3 lm = DecodeLightmap(UNITY_SAMPLE_TEX2D(unity_Lightmap, i.texcoord1));
                col.rgb *= lm;
                clip(col.a - _Cutoff);
